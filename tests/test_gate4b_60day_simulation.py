"""Gate 4B：60 日历史回放预检（historical replay pre-check）。

评审要求（docs/review-gates.md Gate 4）：
「连续模拟 60 个交易日无静默错误、重复订单或无法解释的权益变化。」

**重要边界**：本测试是**历史回放预检**（单进程内用确定性合成行情回放 60 个
连续交易日），验证管线在 60 天尺度下的账务与幂等链路；它**不等于** Gate 4B
「连续 60 个交易日自动运行」验收。正式验收须从真实自动任务启动日起，按实际
交易日经 CLI / Windows 任务计划、跨日进程重启、真实数据更新、失败告警与恢复
流程累计 60 天（进度由 ``scripts/gate4b_observation.py --mode track`` 实时累计）。
...
重复订单与权益变化断言并非空转。该样本量足以证明账务链路能运行，但不构成
长期稳定性结论——长期稳定性由真实 continuous operation 观察给出。
"""
